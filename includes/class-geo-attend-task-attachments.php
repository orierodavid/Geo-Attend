...
class Geo_Attend_Task_Attachments {
    ...
    private static function table() {
        global $wpdb;
        return $wpdb->prefix . 'geo_attend_task_attachments';
    }
    ...
    public static function create_task_with_attachments() {
        ...
        global $wpdb;
        $table = Geo_Attend_DB::tables()['tasks'];
        ...
        $files = isset( $_FILES['geo_task_attachments'] ) ? $_FILES['geo_task_attachments'] : array();
        if ( ! empty( $files ) ) self::store_files( $task_id, $files, get_current_user_id() );
    ...
    private static function store_files( $task_id, $files, $user_id ) {
        ...
        if ( $count > self::MAX_FILES ) wp_die( 'You can attach a maximum of 5 files to one task.' );
        ...
        foreach ( $files['name'] as $index => $original ) {
            ...
            if ( $size < 1 || $size > self::MAX_BYTES ) wp_die( 'Each task attachment must be 10 MB or smaller.' );
            $error = isset( $files['error'][ $index ] ) ? (int) $files['error'][ $index ] : UPLOAD_ERR_NO_FILE;
            if ( UPLOAD_ERR_OK !== $error ) wp_die( 'One of the task attachments could not be uploaded.' );
            $tmp = isset( $files['tmp_name'][ $index ] ) ? $files['tmp_name'][ $index ] : '';
            if ( ! $tmp || ! is_uploaded_file( $tmp ) ) wp_die( 'Invalid task attachment upload.' );
            $type = wp_check_filetype_and_ext( $tmp, $original, $allowed );
            if ( empty( $type['type'] ) || empty( $type['ext'] ) ) wp_die( 'Unsupported task attachment type. Use PDF, Word, Excel, PowerPoint, CSV, TXT, JPG, PNG, WEBP or ZIP.' );
            $single = array( 'name' => sanitize_file_name( $original ), 'type' => $files['type'][ $index ] ?? $type['type'], 'tmp_name' => $tmp, 'error' => $error, 'size' => $size );
            $result = self::move_private_file( $single, $allowed );
            if ( is_wp_error( $result ) ) wp_die( esc_html( $result->get_error_message() ) );
            global $wpdb;
            $wpdb->insert( self::table(), array( 'task_id' => $task_id, 'uploaded_by' => $user_id, 'original_name' => sanitize_text_field( $original ), 'stored_name' => $result['file'], 'relative_path' => $result['relative'], 'mime_type' => $type['type'], 'file_size' => $size, 'created_at' => current_time( 'mysql' ) ), array( '%d','%d','%s','%s','%s','%s','%d','%s' ) );
        }
    }
    ...
    private static function move_private_file( $file, $mimes ) {
        ...
        $name = wp_unique_filename( $base, $file['name'] );
        $target = trailingslashit( $base ) . $name;
        if ( ! @move_uploaded_file( $file['tmp_name'], $target ) ) return new WP_Error( 'move', 'Unable to store the task attachment.' );
    ...
    public static function rest_list( WP_REST_Request $request ) {
        ...
        $task = self::task_for_user( $task_id, $user_id );
        if ( ! $task && ! self::can_manage() ) return new WP_Error( 'forbidden', 'You do not have access to this task.', array( 'status' => 403 ) );
        ...
        foreach ( $rows as &$row ) { $row['size_label'] = size_format( (int) $row['file_size'] ); $row['download_url'] = rest_url( self::REST_NS . '/staff/tasks/' . $task_id . '/attachments/' . (int) $row['id'] . '/download' ); }
    ...
    public static function admin_attachment_field() {
        if ( ! current_user_can( 'manage_options' ) || empty( $_GET['page'] ) || 'geo-attend-tasks' !== sanitize_key( $_GET['page'] ) ) return;
