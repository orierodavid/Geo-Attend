...
class Geo_Attend_Public {
    ...
    public static function config() {
        ...
        return rest_ensure_response( array( 'organization_logo' => esc_url_raw( $settings['organization_logo'] ), 'schedule' => array( 'days' => $days, 'start' => $start_time, 'end' => $end_time, 'timezone' => $settings['timezone'] ), 'open' => $open ) );
    }
    public static function check_in( WP_REST_Request $request ) {
        ...
        $ok = $wpdb->insert( $t['attendance'], array( 'member_id' => (int) $member['id'], 'department_id' => $member['department_id'] ? (int) $member['department_id'] : null, 'location_id' => (int) $location['id'], 'attendance_date' => $date, 'checked_in_at' => $now->format( 'Y-m-d H:i:s' ), 'latitude' => $lat, 'longitude' => $lng, 'accuracy' => $accuracy, 'status' => $status ), array( '%d','%d','%d','%s','%s','%f','%f','%f','%s' ) );
