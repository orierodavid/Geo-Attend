...
class Geo_Attend_DB {
    ...
    public static function activate() {
        ...
        $sql[] = "CREATE TABLE {$t['members']} (id bigint(20) unsigned NOT NULL AUTO_INCREMENT,department_id bigint(20) unsigned NULL,first_name varchar(100) NOT NULL,last_name varchar(100) NOT NULL,pin_hash varchar(255) NOT NULL,active tinyint(1) NOT NULL DEFAULT 1,created_at datetime NOT NULL,updated_at datetime NOT NULL,PRIMARY KEY (id),KEY department_id (department_id),KEY active (active),KEY name_lookup (last_name, first_name)) $charset;";
    ...
    /** Normalize legacy 12-hour and standard 24-hour values to H:i. */
    public static function normalize_time( $value, $fallback = '00:00' ) {
        $value = trim( (string) $value );
        $formats = array( 'H:i', 'H:i:s', 'g:i A', 'g:i a', 'h:i A', 'h:i a', 'g A', 'g a', 'h A', 'h a' );
        foreach ( $formats as $format ) {
            $dt = DateTimeImmutable::createFromFormat( '!' . $format, $value );
            $errors = DateTimeImmutable::getLastErrors();
            $has_errors = is_array( $errors ) && ( $errors['warning_count'] || $errors['error_count'] );
            if ( $dt && ! $has_errors ) return $dt->format( 'H:i' );
        }
        return $fallback;
    }

    public static function schedule_time( $date, $value, $timezone ) {
        $normalized = self::normalize_time( $value );
        try { $zone = new DateTimeZone( $timezone ); } catch ( Exception $e ) { $zone = wp_timezone(); }
        return DateTimeImmutable::createFromFormat( '!Y-m-d H:i', $date . ' ' . $normalized, $zone );
    }
