  ...
  function app() {
    ...
    root.innerHTML = `
      <div class="geo-attend-card">
        <div class="geo-attend-brand">
          ${c.organization_logo ? `<img src="${escapeHtml(c.organization_logo)}" alt="">` : '<span class="geo-attend-mark">G</span>'}
          <div><strong>ATTENDANCE</strong><small>Member Check-in</small></div>
        </div>
        <div class="geo-attend-status ${c.open ? 'is-open' : 'is-closed'}">${c.open ? 'CHECK-IN OPEN' : 'CHECK-IN CLOSED'}</div>
        <div class="geo-attend-intro"><span>ATTENDANCE</span><h2>${c.open ? 'Mark your attendance' : 'Check-in is currently closed'}</h2><p>${c.open ? `Enter your name and PIN, then verify your attendance location.` : `Available ${escapeHtml(days.join(', '))} between ${escapeHtml(start)} and ${escapeHtml(end)}.`}</p></div>
        ${c.open ? `
        <form class="geo-attend-form" id="geo-attend-form">
          ...
          <div class="geo-location-status" id="geo-location-status">Your location will be verified automatically when you check in.</div>
    ...
    form.addEventListener('submit', async function (event) {
      ...
      navigator.geolocation.getCurrentPosition(async function (position) {
        ...
        try {
          const response = await fetch(api + '/check-in', { method: 'POST', headers: {'Content-Type':'application/json','X-WP-Nonce':window.GeoAttend.nonce}, body: JSON.stringify({ member_name:memberName.value.trim(), pin:pin.value, lat:position.coords.latitude, lng:position.coords.longitude, accuracy:position.coords.accuracy }) });
